...
def collect_label_paths(node, path=[], label_paths={}):
    ...
    # 继续遍历子节点
    collect_label_paths(node.left, current_path, label_paths)
    collect_label_paths(node.right, current_path, label_paths)
...
def DFS(root, label_paths, current_path):
    ...
    # 如果是叶子节点，执行分类
    if root.left is None and root.right is None and not is_english_name(root.name):
        classification[root.name] = classify_node(current_path, label_paths)
    else:
        # 递归调用子节点
        DFS(root.left, label_paths, current_path.copy())
        DFS(root.right, label_paths, current_path.copy())
